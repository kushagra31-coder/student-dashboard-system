...
public class projectservlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBconnection.getConnection();
                PreparedStatement stmt = conn.prepareStatement("SELECT title, description FROM projects WHERE student_id = ?")) {
            stmt.setInt(1, studentId);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                projects.add(new String[] { rs.getString("title"), rs.getString("description") });
            }
        } catch (Exception e) {
            request.setAttribute("error", "Unable to load project records: " + e.getMessage());
        }
